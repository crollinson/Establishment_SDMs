...
respvar = 'IV.log' # name of response variable: percentage of trees in the plot established in a given year
...
for(s in species){
  ...
  attach(incsvf)
  print(summary(incsvf[,c("PlotID", "Spp", "IV.avg", "IV.log", "Tmean.yr", "Precip.yr")]))
  print(dim(incsvf[]))
  ...
  varImpPlot(rf.mod)
  vipltn = paste0('Figures/RF_Estab_log/Current_Estab_',s, '_varimplot','.png')
  dev.print(png, vipltn, width=550, height=600)
  
  # MSE Plot -> Model_rf_mseplot.png
  mse = rf.mod$mse
  plot(mse, type='l', xlab="no. of trees", ylab="MSE(OOB)")
  msepltn = paste('Figures/RF_Estab_log/Current_Estab_',s,'_mseplot','.png',sep="")
  dev.print(png, msepltn, width=550, height=600)
  
  # Variable Importance table -> Model_rf_vitable.csv
  vitabn = paste('Data/analyses/RF_Estab_log/Current_Estab_',s,'_vitable','.csv',sep="")
  sink(vitabn)
  ...
  rf_sumf = paste('Data/analyses/RF_Estab_log/Current_Estab_',s,'_summ','.txt',sep="")
  sink(rf_sumf)
  print(rf.mod$call)
  print("R-Square")
  print(rf_rsq)
