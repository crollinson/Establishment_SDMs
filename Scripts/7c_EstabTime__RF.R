...
RF_data <- read.csv('Data/model_inputs/Establishment_SpeciesGroup_Climate_1km_smooth_1900-2013_RF.csv') # Data file

selLst = scan('Data/model_inputs/selvar_Estab10yr.txt',what='string') # List of selected predictors; this is held constant across species
#nwdatv = read.csv('Cover_Climate_pred.csv') # Future climate predictors
respvar = 'p.smooth' # name of response variable: percentage of trees in the plot established in a given year
gisid = 'PlotID.Year' # GIS-ID variable to map results - this is field should exist in the shapefile/coverage; Note: I'm just using it to tag indvidiual observations
...
rf_sumf = paste('Data/analyses/RF_EstabTime/Estab_10yr_',s,'_summ','.txt',sep="")
sink(rf_sumf)
print(rf.mod$call)
print("R-Square")
print(rf_rsq)
